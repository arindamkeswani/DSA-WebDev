...
public class Main {
   ...
   public static void multisolver(ArrayList<Edge>[] graph, int vtx, int dest, boolean[] visited, int criteria, int k, String psf, int wsf) {
       if(vtx==dest){
              if(wsf< spathwt){ //smallest weighted path weight 
                  spathwt=wsf;
                  spath = psf;
              }
              
              if(wsf>lpathwt){ //largest weighted path weight
                  lpathwt=wsf;
                  lpath = psf;
              }
              
              if(pq.size()<k){ //is list till k elements is not complete
                  pq.add(new Pair(wsf, psf)); //add another element
              }else{
                  if(pq.peek().wsf<wsf){ //replace smaller elements to keep k size constant
                      pq.remove();
                      pq.add(new Pair(wsf, psf));
                  }
              }
              
              if(wsf>criteria && wsf<cpathwt){ //nearest larger value
                  cpathwt = wsf;
                  cpath = psf;
              }
              if(wsf<criteria && wsf>fpathwt){ //nearest smaller value
                  fpathwt = wsf;
                  fpath = psf;
              }
              return; //termniate process
          }
       visited[vtx]=true;
