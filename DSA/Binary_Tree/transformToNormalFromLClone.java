...
public class Main {
  ...
  public static Node transBackFromLeftClonedTree(Node node){
    ...
    Node lcr = transBackFromLeftClonedTree(node.left.left); //skip left node and get its left (which is the rest of the left sub-tree)
    Node rcr = transBackFromLeftClonedTree(node.right); //get right sub-tree
    
    // Node nn=new Node(node.data, lcr, null);
    node.left.left=null;//detach the clone node, which will be collected by garbage collector
    node.left=lcr; //attach left subtree to node
    node.right=rcr; //attach right subtree to node
