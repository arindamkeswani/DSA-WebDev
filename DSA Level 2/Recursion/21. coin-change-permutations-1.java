...
public class Main {
    ...
    public static void coinChange(int[] coins, int amtsf, int tamt, String asf, boolean[] used){
       ...
       for(int i=0;i<used.length;i++){ //will go through un-used coins to check all permutations. In combination, once selected, we dont revisit it, but in permutation, we visit all again
           if(used[i]==false){
               used[i]=true; //mark as used
               coinChange(coins, amtsf+coins[i],tamt, asf+coins[i]+"-",used);
               used[i]=false; //mark as un-used
           }
       }
    }
